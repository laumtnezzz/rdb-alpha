...
echo "$($PSQL "SELECT DISTINCT name FROM teams INNER JOIN games ON teams.team_id IN (winner_id, opponent_id) WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"
